...
def reconstruct_signal_griffin_lim(magnitude_spectrogram, fft_size, hopsamp, iterations):
    ...
    while n > 0:
        ...
        #x_reconstruct_new = istft_for_reconstruction(proposal_spectrogram, fft_size, hopsamp)
        ## ATI modified: change to 1/iter step size
        #x_reconstruct = (iterations-n)/(iterations-n+1.)*x_reconstruct + 1./(iterations-n+1.)*x_reconstruct_new
        ...
        if n%100 == 0:
            print('Reconstruction iteration #{} of {} RMSE: {} '.format(iterations - n, iterations, diff))
